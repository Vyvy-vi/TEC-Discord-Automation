...
def main(key: int, title: str, text: str, img: str, url: str, time: str) -> None:
    """Function to send embed with data to discord channel"""
        box = nacl.secret.SecretBox(key)
        url = box.decrypt(bytes(url, 'utf-8'), encoder=nacl.encoding.HexEncoder).decode('utf-8')
        hook = Webhook(url)
        embed = Embed(
                title = f"**{title}**",
                description=text,
...
if __name__ == '__main__':
    ...
    IMG = "https://raw.githubusercontent.com/Vyvy-vi/TEC-Discord-Automation/main/.github/resources/" + os.environ['IMG']
